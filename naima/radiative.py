...
class BaseRadiative(object):
    ...
    def __init__(self, particle_distribution):
        ...
        try:
            ...
            validate_physical_type(
                "Particle distribution", pd, physical_type="differential energy"
            )
...
class BaseElectron(BaseRadiative):
    ...
    def _gam(self):
        ...
        return np.logspace(
            log10gmin, log10gmax, self.nEed * (log10gmax - log10gmin)
        )
...
class Synchrotron(BaseElectron):
    ...
    def _spectrum(self, photon_energy):
        ...
        # Critical energy, erg
        Ec = (
            3 * e.value * hbar.cgs.value * self.B.to("G").value * self._gam ** 2
        )
...
class Bremsstrahlung(BaseElectron):
    ...
    def _emiss_ee(self, Eph):
        ...
        emiss = c.cgs * trapz_loglog(
            np.vstack(self._nelec) * self._sigma_ee(gam, Eph), self._gam, axis=0
        )
        return emiss
    ...
    def _emiss_ep(self, Eph):
        ...
        # compute integral with electron distribution
        emiss = c.cgs * trapz_loglog(
            np.vstack(self._nelec) * self._sigma_ep(gam, eps), self._gam, axis=0
        ).to(u.cm ** 2 / Eph.unit)
        return emiss
...
class PionDecay(BaseProton):
    ...
    def _nuclear_factor(self, Tp):
        ...
        epstotal = np.where(
            Tp > self._Tth, epsC + (eps1 + eps2) * sigmaRpp * G / sigmainel, 0.0
        )
